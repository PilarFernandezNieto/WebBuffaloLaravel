@php
    $tabs = [
        ...
        [
            ...
            'active' => request()->routeIs('admin.contenidos.*'),
        ],
        [
            'label' => 'Productos',
            ...
            'active' =>
                ...
                request()->routeIs('admin.discos.*') ||
                request()->routeIs('admin.camisetas.*'),
        ],
    ];
@endphp
<nav aria-label="Secciones"
    ...
    @foreach ($tabs as $tab)
        <a href="{{ route($tab['route']) }}" @if ($tab['active']) aria-current="page" @endif
            class="flex items-center min-h-12 px-[clamp(12px,2vw,18px)] font-cuerpo text-sm tracking-[1px] uppercase border-b-[3px] transition duration-200 {{ $tab['active'] ? 'font-bold text-ink-heading border-oxide-light' : 'font-semibold text-ink-muted border-transparent hover:text-ink-heading' }}">
            {{ $tab['label'] }}
        </a>
    @endforeach
</nav>
