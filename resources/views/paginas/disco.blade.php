<x-app-layout pageTitle="{!! $disco->nombre !!}">
    ...
    <div class="max-w-content mx-auto px-edge pt-section">
        ...
        <div class="flex flex-wrap gap-cols-lg items-start pb-section">
            ...
            <div class="flex-1 min-w-[300px]">
                <p class="font-titulo italic text-oxide text-[13px] font-semibold uppercase tracking-[1.2px]">
                    {{ $disco->anio_edicion }}@if ($disco->sello) &middot; {{ $disco->sello }} @endif
                </p>
                <h1 class="font-titulo font-black uppercase text-ink-heading text-[clamp(30px,3.8vw,46px)] leading-[1] tracking-[-0.025em] mt-2 mb-4">
                    {{ $disco->nombre }}
                </h1>
                @php
    ...
    @php
        ...
        if ($disco->informacion) {
            ...
            $dom->loadHTML('<?xml encoding="utf-8" ?>' . $disco->informacion);
            libxml_clear_errors();
    ...
    @if ($personal || $creditos)
        <div class="max-w-content mx-auto px-edge pb-section flex flex-wrap gap-cols-lg items-start border-t border-rule-light pt-cols">

            @if ($personal)
                <div class="flex-1 min-w-[300px] basis-[380px]">
                    <h2 class="font-titulo font-black uppercase text-ink-heading text-[clamp(18px,2vw,22px)] tracking-[-0.01em] mb-5">Personal</h2>
                    <ul class="list-none m-0 p-0 flex flex-col gap-3.5">
                        @foreach ($personal as $p)
                            <li class="flex flex-wrap gap-x-4 gap-y-1 items-baseline pb-3.5 border-b border-rule-light-soft">
                                <span class="flex-1 min-w-[190px] text-[15px] font-bold text-ink-heading">{{ $p['nombre'] }}</span>
                                <span class="flex-1 min-w-[200px] text-sm leading-[1.55] text-ink-muted">{{ $p['rol'] }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            @if ($creditos)
                <div class="flex-1 min-w-[280px] basis-[300px]">
                    <h2 class="font-titulo font-black uppercase text-ink-heading text-[clamp(18px,2vw,22px)] tracking-[-0.01em] mb-5">Créditos</h2>
                    <dl class="m-0 flex flex-col gap-4">
                        @foreach ($creditos as $c)
                            <div>
                                @if ($c['etiqueta'])
                                    <dt class="text-[11px] font-bold uppercase tracking-[1.3px] text-ink-muted mb-1">{{ $c['etiqueta'] }}</dt>
                                @endif
                                <dd class="m-0 text-[15px] leading-[1.65] text-ink-body">{{ $c['valor'] }}</dd>
                            </div>
                        @endforeach
                    </dl>
                </div>
    ...
    <section class="bg-ink">
        <div class="max-w-content mx-auto px-edge py-section flex flex-wrap gap-cols-lg">
            ...
            <div class="flex-1 min-w-[300px] informacion-oscura text-onblack-body text-[15px] leading-[1.75] max-w-[70ch] space-y-4">
                {!! $disco->textos !!}
            </div>
        </div>
    </section>
    ...
    <div class="max-w-content mx-auto px-edge py-section">
        ...
        @endphp
        @if ($otros->count())
            <p class="font-titulo italic text-oxide text-[13px] font-semibold uppercase tracking-[1.2px] mb-5">Otros discos</p>
            <div class="flex flex-wrap gap-cols mb-10">
                @foreach ($otros as $otro)
                    <a href="{{ route('discografia.mostrar', $otro->slug) }}"
                        ...
                        <div class="min-w-0">
                            <p class="font-titulo font-black uppercase text-ink-heading text-base tracking-[-0.01em]">{{ $otro->nombre }}</p>
                            <p class="text-ink-muted text-xs font-semibold uppercase tracking-[1px] mt-1">{{ $otro->anio_edicion }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
