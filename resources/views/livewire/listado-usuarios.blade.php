        ...
        <tbody>
            @forelse ($usuarios as $usuario)
                <tr class="border-b border-rule-light-soft last:border-b-0">
                    <td class="px-4 py-3.5 text-[13px] font-bold text-ink-muted whitespace-nowrap">{{ $usuario->id }}
                    </td>
                    <td class="px-4 py-3.5 min-w-[180px]">
                        <span
                            class="block text-[15px] font-bold text-ink-heading text-pretty">{{ $usuario->name }}</span>
                    </td>
                    <td class="px-4 py-3.5 text-sm text-ink-body break-words">{{ $usuario->email }}</td>
                    <td class="px-4 py-3.5">
                        <span
                            class="text-[11px] font-bold tracking-[1.2px] uppercase text-ink-muted border border-rule-input px-[10px] py-[5px] rounded-sharp whitespace-nowrap">
                            {{ $usuario->admin ? 'Administrador' : 'Usuario' }}
                        </span>
                    </td>
                    <td class="px-4 py-3.5 text-[13px] font-semibold text-ink-muted whitespace-nowrap">
