    ...
    <table class="w-full min-w-[640px] border-collapse text-left">
        <thead>
            <tr class="bg-cream-hover">
                ...
                <th class="text-[11px] font-bold tracking-[1.3px] uppercase text-ink-muted px-4 py-3.5 border-b border-rule-light whitespace-nowrap">Categoría</th>
        ...
        <tbody>
            @forelse ($productos as $producto)
                <tr class="border-b border-rule-light-soft last:border-b-0">
                    <td class="px-4 py-3.5 text-[13px] font-bold text-ink-muted whitespace-nowrap">{{ $producto->id }}</td>
                    <td class="px-4 py-3.5 min-w-[180px]">
                        <span class="block text-[15px] font-bold text-ink-heading text-pretty">{{ $producto->nombre }}</span>
                    </td>
                    <td class="px-4 py-3.5 text-sm text-ink-body whitespace-nowrap">{{ $producto->categoria->nombre }}</td>
                    ...
                    <td class="px-4 py-3.5 text-sm font-semibold text-ink-body whitespace-nowrap">{{ $producto->precio_formateado }}</td>
                    <td class="px-4 py-3.5 whitespace-nowrap">
                        <div class="flex gap-3.5 items-center">
                            ...
                            <button type="button" wire:click="$dispatch('mostrarAlerta',{{ $producto->id }})"
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-6 text-sm text-ink-muted text-center">No hay productos todavía.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
